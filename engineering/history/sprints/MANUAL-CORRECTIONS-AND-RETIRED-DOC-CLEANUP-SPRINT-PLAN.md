...
| §3 | F-256 | **ANSWERED — and it was a DOCUMENTATION answer, not a bench one (see F-273).** `_RET_` executes the instruction and returns **only if it did not branch** (P2 Assembly Language Manual 2022-11-01 p.68; P2 Instructions v35 Rev B/C row 410). `CALL` branches, so `_RET_ CALL` never returns — **specified behaviour, not a silicon quirk and not a compiler bug.** The bench corroborated it; our KB had dropped the qualifier, which is the real defect. Execution falls out of the routine into whatever follows in cog RAM; **dispatch is not lost** (the rig's spurious handler restored it), so the failure mode is silent EXTRA execution. **§4 therefore needs restructuring, not just a `set_nz` definition** — exactly the contingency Open Question 2 flagged. |
