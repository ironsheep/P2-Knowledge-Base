...
# Pattern for table of contents entries - look for grouped entries
toc_pattern = r'^([A-Z][A-Z0-9]+(?:\s*/\s*[A-Z0-9]+)*)\s+\d+'
# Also look for items in the body
body_pattern = r'^([A-Z][A-Z0-9]+(?:\s*/\s*[A-Z0-9]+)*)\s+D\s*,.*\d+'

for line in manual_content.splitlines():
    # Check table of contents
    toc_match = re.match(toc_pattern, line)
    if toc_match:
        entry = toc_match.group(1)
        # Handle grouped instructions like "AND / ANDN"
        if ' / ' in entry:
            parts = [p.strip() for p in entry.split('/')]
            # All parts are documented together
            for part in parts:
                manual_items.add(part)
                manual_groups[part] = parts  # Track that these are grouped
        else:
            manual_items.add(entry)
    
    # Check body entries
    body_match = re.match(body_pattern, line)
    if body_match:
        entry = body_match.group(1)
        # Handle grouped instructions
        if '/' in entry:
            parts = [p.strip() for p in entry.split('/')]
            for part in parts:
                manual_items.add(part)
                manual_groups[part] = parts
        else:
            manual_items.add(entry)
...
print(f"Found {len(manual_groups)} grouped instructions")
print(f"Known directives: {len(KNOWN_DIRECTIVES)}")

# Debug: Show some grouped instructions
if manual_groups:
    print("\nExample grouped instructions:")
    for key, group in list(manual_groups.items())[:5]:
        print(f"  {key} -> {group}")
...
# Show grouped instructions
grouped_in_manual = [item for item, data in yaml_items.items() if data.get('is_grouped_in_manual')]
if grouped_in_manual:
    print(f"\n=== Grouped Instructions in Manual ({len(grouped_in_manual)}) ===")
    for item in sorted(grouped_in_manual)[:10]:
        group = yaml_items[item]['manual_group_members']
        print(f"  {item} -> grouped with {group}")
...
output_data = {
    ...
    'statistics': {
        ...
        'in_both': len(manual_items & set(yaml_items.keys())),
        'missing_yaml': len(missing_yaml),
        'extra_yaml': len(extra_yaml),
...
print(f"\nAnalysis saved to instruction_analysis.yaml")
print(f"Heat map can be generated with generate_heat_maps.py")