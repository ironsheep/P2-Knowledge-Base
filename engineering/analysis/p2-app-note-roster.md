...
| C4 | **Diagnosing Behavioural Instability in a Multi-Cog Rig** *(candidate, raised 2026-08-30)* | **C3's successor: from sizing ONE stack to supervising a RIG.** The topology in actual use — a driver in a back cog, several front cogs exercising it, and **cog 0 kept alive as the supervisor** watching the front cogs' stack buffers and still able to report when they lock up. Composition, not reference: it combines the Spin2 method stack (F-392), the DEBUG system's resource contract (F-393), inline-PASM parking as a control (F-394), and cross-cog observability limits. **Self-reinforcing — the instrument it uses is C3's own `isp_stack_check`**, so the note cites a published P2 Knowledge Base note the way C3 cites C1. **Three readings per front cog, and they are different instruments:** alive (hub heartbeat counter — `COGCHK` returns *running*, and a hung cog is still running, v55:520), depth (high-water from the fill pattern — `PTRA` is private, silicon-doc:137/:284), boundary (sentinel). No clean P1 analogue; P2-unique because of 8 cogs + the debug interrupt. | `isp_stack_check` (already shipped in C3's example library); the park idiom `org`/`jmp #$`/`end` (v55:312); `COGBRK` escalation (silicon-doc:2520-2524) | **CANDIDATE — direction agreed with Stephen 2026-08-30; not yet committed to production, no `PUBLICATION-ROSTER` row, no ID allocated (P2AN008 is next free)** |
