...
last_published_tag: p2an006-v1.0.1   # baseline for Dimension #15 (released 2026-08-08, 13pp)
guide_paths:
...
fragile_areas:
  ...
  - "P2 operator is `<=` not `=<` (pnut-ts rejects `=<` with 'Expected end of line') — caught during authoring"
---
...
- **Grounding model:** `reference` — verify against `cogspin.yaml` / `taskspin.yaml` (stack params),
  ...
  stack-packing text, `pnut-ts` v1.55.0, and the shipped `isp_stack_check.spin2` (MIT). No P1
...
- **Verification model:** every embedded block + every `examples-library/*.spin2` compiles under
  `pnut-ts -d` (each OBJ-includes `isp_stack_check.spin2`; the utility uses `debug()`). Overflow
