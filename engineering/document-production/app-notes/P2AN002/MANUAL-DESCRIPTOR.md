...
last_published_tag: p2an002-v1.0.3   # baseline for Dimension #15 (released 2026-08-17, 15pp)
guide_paths:
...
authoritative_sources: see ../APP-NOTE-CREATION-GUIDE.md §5.1 # P2 Documentation v35 (CORDIC Solver section) + P2 Datasheet + Spin2 docs (ROTXY/POLXY/XYPOL/QSIN/QCOS/MULDIV64) + PASM2 Reference (companion manual) + OBEX (#2811/#2812/#5278/#5361) + pnut-ts
high_risk_quant:
...
- **Grounding model:** `reference` — verify against `architecture/cordic.yaml`, the PASM2
  ...
  (`rotxy/polxy/xypol/qsin/qcos/muldiv64`), and the **Parallax Propeller 2 Documentation v35 - Rev B/C** CORDIC Solver section.
...
- **Code (Dimensions #3/#3b):** every embedded block + every `examples-library/*.spin2` compiles
  under `pnut-ts` (`-d`, uses `debug()`); K=76; inline code ASCII-only.
