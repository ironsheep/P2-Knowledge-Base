...
PRI worker() | cmd_t snap, lastSeq
  ...
  repeat
    ...
    if snap.seq <> lastSeq                ' new command? ask the SNAPSHOT
      lastSeq := snap.seq
      debug("worker exec op=", udec(snap.opcode), " arg=", udec(snap.arg), ...
            " seq=", udec(snap.seq))
```
...
⚠ **Pitfall — one long is not automatically one store.** Writing `cmd.opcode := i` then `cmd.arg := …` then `cmd.seq := …` *directly into the shared record* is three separate read-modify-writes of that long, and a reader can catch it between any two of them — the exact torn read you packed the record to avoid. Fitting in one long is what makes an atomic publish *possible*; staging the record privately and storing it in **one** statement is what makes it *happen*. The read side has the same trap: snapshot once, then read fields from the snapshot.
...
💡 **Tip — brackets write the pointer, the bare name writes the record.** `[ph] := @packet` aims the pointer at the buffer. Plain `ph := …` means "assign to the record it points at," and the compiler will reject it — the same bracket rule as the child-object pitfall below.
...
- **Example library** — every recipe as a standalone, `pnut_ts`-compiled program: `in-cog-record.spin2` (R1), `spsc-ring-buffer.spin2` (R2), `latest-wins-mailbox.spin2` (R3), `locked-multiwriter-queue.spin2` (R4), `single-long-packed-record.spin2` (R5), `packed-header-offsets.spin2` (R6). Published as `P2AN007-src-<date>.zip` beside this note.
...
- **v1.0.0** (July 2026) — first public release. Six runnable recipes, each `pnut_ts -d`-compiled. Adds coverage of the two STRUCT extensions released since v45: **member bitfields** (v54) as R5, a whole command record packed into a single atomically-published long, and **`OFFSETOF`** (v53) as R6, computed offsets for raw addressing. Recipes R1–R4 remain `{Spin2_v45}`, so a compiler predating those extensions still runs them. Implementation-only by design; the contract-choice reasoning stays with the P2 Architect's Guide.
  <!-- HOLD until the two-cog hardware run certifies: append the race-freedom confirmation sentence here, citing its EF ledger entry. Do not ship this line before the logs exist. -->

- **v0.1.0** (July 2026) — initial draft for review. Four runnable recipes (in-cog record, lock-free SPSC ring buffer, latest-wins mailbox, locked multi-writer queue), each `pnut_ts -d`-compiled.
