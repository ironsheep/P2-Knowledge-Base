...
| **P2AN002** | 2026-08-17, at v1.0.3 | Register blend and markers clean; R1 exemplary throughout — "roughly seven" is the model: a hedge that names its own arithmetic (54 stages / 8-clock window). **Two of the three phrases originally quoted here have been withdrawn, and the reason is the lesson: a good-sounding hedge is not the same as a sourced one.** *"about 28 bits"* read as careful precision and was **retired in v1.0.4** — no Parallax source states a bit count for the iterative residual, so the note now tells the reader to measure it against their own tolerance. Do not restore it, here or in another note. *"measured on real P2 silicon"* never appeared in the shipped PDF at either version — it was quoted from somewhere other than the artifact, which is the same failure this guide exists to catch. Checked against the rendered v1.0.3 and v1.0.4 text, 2026-09-11. Four backticked mnemonics normalised to the bare uppercase form of §5. Every OBEX number, title, and author verified against the live catalog — one Acknowledgments attribution (#2812) named a company where Resources and the catalog both say **ersmith**, now consistent. |
