...
With `CODE = $4000` the program reports **825000 µV** (0.825 V, a quarter of 3.3 V), and a P2AN001 ADC on the jumpered pin should read close to it. Build with DEBUG enabled (`pnut-ts -d`) so the printed line appears.
...
- **Nothing printed by the Tier-0 program.** Build with DEBUG enabled (`pnut-ts -d`); without it the `debug()` directive is ignored.
...
- **Example code (download).** The five worked builds in this note — sample playback, waveform synthesis, dithering, ADC passthrough, and the stereo mixer — are published together as the **P2AN003 source ZIP** beside this note's PDF, each a complete `pnut-ts`-compiled program ready to build and run.
