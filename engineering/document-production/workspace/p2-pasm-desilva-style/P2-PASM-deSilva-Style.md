...
REP comes in two flavors - count form and label form:

```pasm2
' Count form: REP #instructions, #iterations
...
' Label form: REP @.end, #iterations (more maintainable!)
        rep     @.end, #5      ' Repeat until .end, 5 times
        add     x, #1
        sub     y, #1
        rol     z, #1
        ror     w, #1
.end    ' Compiler counts instructions automatically
```

The label form is often better - add or remove instructions without changing the count!
...
```pasm2
        setq    ##128-1          ' Transfer 128 longs to COG
        rdlong  buffer, source   ' Happens at maximum speed!
        ' 512 bytes moved in microseconds!
```
