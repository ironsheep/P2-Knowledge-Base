...
What you don't see below the surface: Nine specialized window types. Interactive controls. Real-time visualization. Performance monitoring without external tools. Bidirectional communication with your PC. Graphics that update 20x faster than you thought possible.
...
::: needs-screenshot
**Description:** First PLOT window example showing real-time data plotting  
**Dimensions:** 256x256  
**Window Type:** PLOT  
**Content:** Live sine wave being drawn point by point
:::
...
::: needs-screenshot
**Description:** Professional multi-window debugging setup  
**Dimensions:** 800x400  
**Window Type:** MULTI  
**Content:** Three synchronized windows - terminal for serial monitoring, scope for analog signals, logic for digital signals
:::
...
::: needs-screenshot
**Description:** SCOPE_XY revealing motor glitch  
**Dimensions:** 256x256  
**Window Type:** SCOPE_XY  
**Content:** X-Y plot showing normal operation as tight cluster, with obvious outlier points revealing overcurrent spikes
:::
...
connect_jtag()      ' [NO] Needs hardware debugger
setup_swo_trace()   ' [NO] Requires special pins
configure_etm()     ' [NO] Complex setup

' P2 debugging
DEBUG("Ready!")     ' [OK] Just works
```
:::
...
PUB debug_sensors_pro()
  ...
  repeat
    ...
    DEBUG(`Sensors GOTOXY `(15) `(3) udec_(temperature) "degF   ")
...
PRI update_temperature_field() | temp
  ...
  DEBUG(`Status COLOR `(WHITE10) udec_(temp) "degC  ")
```
:::
...
PRI update_performance_section() | cpu, mem, temp
  ...
  DEBUG(`Dashboard udec_(temp) "degC  ")
...
::: needs-screenshot
**Description:** BITMAP revealing servo resonance  
**Dimensions:** 400x200  
**Window Type:** BITMAP  
**Content:** Graph showing green target line and red actual position with visible oscillations around 47Hz revealing mechanical resonance
:::
...
But we've only scratched the surface. Chapter 4 introduces the revolutionary sprite-based layer system - a technique that provides 20x performance improvement and transforms PLOT windows into professional debugging instruments.
...
*A discovery that changed everything: The sprite-based layer system provides 20x performance improvement through selective updates. This powerful technique, first demonstrated by community member Jon McPhalen, wasn't documented. Until now.*
...
One sprite sheet. Multiple gauge states. No drawing. Just sprite selection. This is 20x faster than calculating and drawing.
...
**20x performance improvement** through sprite updates
...
PUB multi_gauge_dashboard()
  ...
  repeat
    ...
    update_gauge(2, get_temperature(), 0, 250) ' 0-250degF
...
Chapter 7 introduces the performance revolution: packed data formats that accelerate your debugging by 16x.

---

*Next: Chapter 7 - Packed Data Revolution: 16x Compression*
...
The P2's packed data formats solve this crisis with surgical precision. Instead of converting binary to text, transmitting text, then converting back to binary for display, packed formats maintain binary efficiency throughout the pipeline. The result? Up to 32x compression for digital signals, 16x for analog samples, and the difference between unusable and professional debugging.
...
|--------|------------|-------------|-----------------|----------------|
| Text   | 56 (avg)   | 0.14x       | 4 kHz          | 230 kbps      |
| PACK1  | 1          | 32x         | 230 kHz        | 230 kbps      |
| PACK2  | 2          | 16x         | 115 kHz        | 230 kbps      |
| PACK4  | 4          | 8x          | 57 kHz         | 230 kbps      |
| PACK8  | 8          | 4x          | 28 kHz         | 230 kbps      |
| PACK16 | 16         | 2x          | 14 kHz         | 230 kbps      |
...
::: needs-screenshot
**Description:** Complete PID tuning dashboard  
**Dimensions:** 800x800  
**Window Type:** MULTI-PLOT  
**Content:** Four synchronized plots showing setpoint vs process response (top), error signal (bottom-left), control output (bottom-right), and individual P/I/D contributions (bottom strip) - revealing control loop dynamics in real-time
:::
    ' PID calculation
...
|--------|------------|---------------------------|---------|-----------|
| Text   | 0.14x      | 4 kHz                    | High    | Very High |
| PACK1  | 32x        | 2 MHz                    | Low     | Low       |
| PACK2  | 16x        | 1 MHz                    | Low     | Low       |
| PACK4  | 8x         | 500 kHz                  | Low     | Low       |
| PACK8  | 4x         | 250 kHz                  | Low     | Medium    |
| PACK16 | 2x         | 125 kHz                  | Low     | Medium    |
| PACK32 | 1x         | 62.5 kHz                 | Low     | High      |
...
### Formula
```
Bandwidth (bytes/sec) = Sample_Rate x Bits_Per_Sample / 8
```

### Examples
- 1MHz sampling, PACK1: 1,000,000 x 1 / 8 = 125 KB/s
- 100kHz sampling, PACK16: 100,000 x 16 / 8 = 200 KB/s
- 44.1kHz audio, PACK16: 44,100 x 16 / 8 = 88.2 KB/s
...
1. **Text is 10x slower than packed data**
2. **Hardware streaming is 100x more efficient**
3. **Update rate x data size = bandwidth**
4. **Buffer size = rate x duration x size**
5. **Multi-window overhead ~10% per window**
6. **Conditional debug adds <5% overhead**
7. **Separate debug cog eliminates timing impact**
8. **Packed formats reduce bandwidth 2-32x**
...
PUB phase_analyzer() | angle, x, y
  ...
  repeat angle from 0 to 359
    ...
    ' Show phase value
    DEBUG(`TERM "Phase: ", SDEC_(phase_shift), "deg", 13`)
...
| **SCOPE_XY** | `<x>,<y>` or `<r>,<theta>deg` | Cartesian or polar | Phase measurements, Lissajous analysis, vector displays |
...
PUB measure_phase_shift()
  ...
  ' User workflow:
  ' 1. Hover over pattern maximum: read angle as 45deg
