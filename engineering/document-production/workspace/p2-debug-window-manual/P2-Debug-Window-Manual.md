...
The packing keywords come in three families: `LONGS_1BIT` through `LONGS_16BIT`,
`WORDS_1BIT` through `WORDS_8BIT`, and `BYTES_1BIT` through `BYTES_4BIT` — twelve
modes in all. Every mode delivers unsigned (zero-extended) values by
