...
A 9-bit literal Dest is enough to express the base pin (Dest\[5:0\]) and a range of up to 8 contiguous pins (Dest\[8:6\]). If needed, use the augmented literal feature (\textbackslash\{\}\#\textbackslash\{\}\#Dest) to augment Dest to an 11-bit literal value, which inserts an AUGD instruction prior.
...
FLTRND sets the I/O pin(s) designated by Dest to the input direction and with output level(s) set randomly low and high, based on bit(s) from the Xoroshiro128\textbackslash\{\}*\textbackslash\{\}* PRNG. All other pins are left unchanged. This instruction can affect one or more of the bits within the DIRA or DIRB and OUTA or OUTB registers.
