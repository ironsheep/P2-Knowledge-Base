...
- [ ] **LOC moves from Arithmetic to Branch in the categorical index.** `loc.yaml` carried `category: "Math and Logic"`, which is why Appendix C and `instruction-categories.md` group LOC under Arithmetic — the manual was faithfully following its source. The YAML was corrected to `Branch` on 2026-09-22 (F-452, «#351»), matching `calld`/`callpa`/`callpb`/`jmp`. The manual now lags its source by one category on one instruction. **Deliberately not re-staged**: the v3.1.9 PDF is already built, and one category move does not justify a Forge cycle. **Expiry: the next Assembly release**, where Appendix C's Arithmetic and Branch sections and `instruction-categories.md` both need the row moved. Recount the per-category totals when it moves — Appendix C's counts are audited against the YAML.
