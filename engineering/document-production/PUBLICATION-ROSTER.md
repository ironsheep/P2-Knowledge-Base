...
```
2026-07-04 21:15  PUBLISH   p2-io-and-smart-pins-user-guide  (v1.0.1, 396pp — hardware-verified patch: Ch.5 SETQ-armed event-wait timeout — a single WAITxxx stalls on the pin event OR a CT deadline, no busy-poll (F-193/EF-020) — + Ch.15 concurrent period/frequency example routes BOTH A and B inputs to the observed pin so period-aligned cells close their window (F-192/EF-017); render-verified: 396pp = prior, outline complete (19 ch + App A-G), changed sections text-present, compile-clean)
...
| `p2-io-and-smart-pins-user-guide` | ✅ current | **v1.0.1 released 2026-07-04** (396pp); hardware-verified patch (F-193 Ch.5 event-wait timeout + F-192 Ch.15 A+B routing); built on the latest platform (heading-widow figures.lua fix + cross-ref filter pilot) |
