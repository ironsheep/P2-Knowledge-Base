...
```
2026-08-11 22:02  PUBLISH   p2-pasm-desilva-style            (v3.0.5, 164pp — worked-example correctness, BENCH-VERIFIED on real silicon: the Ch.1 fading experiment drives its pin (`P_OE`), the async-serial TX/RX examples assemble (byte/send/recv are reserved words), the quadrature example routes B from the next pin up. Two new Ch.1 asides give the per-board LED pin map (Edge P56/P57 · Edge-32MB P38/P39 · Eval #64000 P56-P63) and explain the buffered-LED touch sensitivity; Ch.14 Quick Reference names P_OE on every output mode. *** RE-CUT IN PLACE at 22:02 (version deliberately held at 3.0.5, tag moved) *** — the 18:56 build shipped a WRONG MECHANISM in the touch-a-pin aside (explained as microamps through the LED; these boards use a Schmitt-trigger LED buffer, so a floating buffer input switches the LED fully ON — corrected against edge-standard/edge-32mb narratives, which document the sensitivity as intended). Same re-cut added the fade's observed ramp-then-snap behavior, the Eval-board LED row, the LED switch position (ON), and reflowed the p131 note. Render-verified: 164pp (= prior), 0 LaTeX errors, 0 blank pages of 164, 0 tofu; p17/p21/p131 EYEBALLED. Codepoint diff vs v3.0.4: 0 newly-introduced non-ASCII — the check added after the 18:19 build shipped a U+2611 tofu glyph.)
