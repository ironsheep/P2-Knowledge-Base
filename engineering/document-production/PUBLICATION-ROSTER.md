...
```
2026-08-11 18:56  PUBLISH   p2-pasm-desilva-style            (v3.0.5, 164pp — worked-example correctness: the Ch.1 fading experiment drives its pin (`P_OE`), the async-serial TX/RX examples assemble (byte/send/recv are reserved words), the quadrature example routes B from the next pin up, and two new Ch.1 asides give the per-board LED pin map + why floating pins glow. Ch.14 Quick Reference now names P_OE. BENCH-VERIFIED on real silicon by Stephen — the fade defect was reader-reported and the fix is confirmed working, not just source-verified. Render-verified: 164pp (was 162, +2 for the new asides — an increase, no drop), 231 outline entries with Chapters 1-16 contiguous + Appendix A + Index, 0 LaTeX errors, 0 blank/near-blank pages of 164. p131 EYEBALLED after a first build shipped a tofu glyph: the U+2611 output-mode marker is absent from this stack's fonts and rendered as an empty box in 5 places — replaced with ASCII [OE] and re-rendered. Codepoint-set diff vs v3.0.4 confirms 0 newly-introduced non-ASCII characters.)
