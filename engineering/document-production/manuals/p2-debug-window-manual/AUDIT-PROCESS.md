...
**Two grounding models — determine which applies from `./creation-guide.md` before starting:**

- **Reference manuals** (instruction / method references — PASM2, Spin2, Smart Pins) are **YAML-backed**. The source of truth is KB YAML under `deliverables/ai/P2/language/`, plus the silicon doc and encoding spreadsheet. Dimensions A, B, #1, #5, and C verify against that YAML.
- **Behavior manuals** (how a subsystem *operates* — Single-Step Debugger, Debug Windows) are **source-extraction-backed**. There is little or no instruction YAML to check against; the source of truth is the reverse-engineered / extracted reference for that subsystem — a *Theory-of-Operations* doc, Pascal/PNut source extractions, or per-feature study docs — **as enumerated in that manual's `./creation-guide.md`**. Dimensions A, B, #1, #5, and C verify against *those* documents, not against `deliverables/ai/P2/language/`. For this manual type, "the YAML" in the dimension descriptions below means "the manual's cited extraction/source documents," and the `p2kb-mcp`/YAML-on-disk caveat does not apply.

For YAML-backed manuals, P2 sources typically cited are:
...
Sixteen dimensions, grouped into six themes. Every dimension produces findings into the consolidated report (§6).
