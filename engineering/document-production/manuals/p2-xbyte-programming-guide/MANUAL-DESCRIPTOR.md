...
high_risk_tables:                                 # transposition-prone — weight heavily on audit  (chapter #s = v1.1.0 restructure: 21 ch / 7 parts)
  - "Ch.11 Table-Size & Compression Modes — the 256/128/64/32/16 (+alt) bit-pattern table; index-calc columns"
  ...
  - "Ch.9 the 8-clock dispatch-cycle table (clock→activity rows)"
...
fragile_areas:                                    # known-thin / discipline-sensitive
  - "Capstone 6502 emulator (Ch.16) + prefixes/alternate-tables (Ch.18, incl. the 6809 SETQ2 pages) — TINY & ILLUSTRATIVE by charter; guest-CPU ISA facts are historical, but all PASM2 must pnut-ts compile"
...
- **Structure (Dimension #10):** 21 chapters in 7 parts — I The Landscape · II Dispatch on the P2 ·
  III Choosing Your Rung · IV The XBYTE Engine · V Building Interpreters and Emulators · VI Reference ·
  VII Appendices (A–D) + clickable Index. Front matter = house standard. Modeled on the Streamer guide.
- **Voice (Dimension #9):** two registers (teaching + reference) per voice-guide. Teaching
  register (Part I, Ch.3, chapter openers) allows "you" + analogy; reference register (tables,
