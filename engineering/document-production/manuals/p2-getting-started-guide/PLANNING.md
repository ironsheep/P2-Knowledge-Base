# Getting Started with the Propeller 2 — Planning Charter (rich-planning phase)

> **Split note (2026-06-24).** This charter was split from the original 4-chapter *P2 Architect's Guide*
> first draft. This folder is now the **Getting Started** orientation book (the on-ramp). The
> decomposition/design-scope planning — functional decomposition, the spatial-computing thesis, the FPGA
> appendices, the decomposition glossary — moved with Ch4 to the design book and now lives in its own charter
> at `manuals/p2-architect-guide/PLANNING.md`. This document was reframed in place to the orientation
> identity; references below to "this guide" mean the **Getting Started** book.

**Status:** DRAFT planning charter · 2026-06-22 (reframed 2026-06-24) · **slug** `p2-getting-started-guide`
...
**Title (D1, approved):** *Getting Started with the Propeller 2*
...
> where the subsystems are introduced *in relation to each other*. This manual is that missing layer: the
> orientation on-ramp that sits below the reference manuals.

---

## 1. The thesis (one sentence)
**This guide is the P2 on-ramp: it teaches a developer — and an AI agent — to picture the P2 architecture as
a mental model, read its code fluently, and put its features to work, so that every reference manual that
follows makes sense.**

## 2. What this is — and emphatically is NOT
- **IS:** the **orientation layer** that sits *below* the reference manuals and ties the
  subsystems together — the on-ramp. A *slim* manual (no length cap — content-driven, kept brief by link-out; D5), not a comprehensive reference.
- **IS NOT:**
  ...
  - a system-design / decomposition book — that's the sibling **P2 Architect's Guide — Designing Real Systems
    on the Propeller 2** (functional decomposition, the design workflow, the spatial-computing thesis live there).
- **Discipline:** every place a subsystem is introduced, the guide **orients then links out** ("smart pins do
  X — the 32 modes are in the Smart Pins manual"). Link-out, never duplicate. This boundary is the manual's
  contract. When the reader is ready to *design* systems, the guide points OUT to the design book.

## 3. Why this is a unique community contribution ("why us")
The community has the silicon-architecture spec (dense), deep topic manuals, and scattered forum lore.
**No one is producing a curated, trust-chain-verified on-ramp — a "picture the P2, read its code, put it to
work" orientation.** We can — because we have the cross-source-verified KB behind it, *and* we have just built
the P1→P2 delta catalogue, so this guide can also serve the large P1-veteran community migrating to P2. An
orientation that ties the split topic-manuals back together *in relation to each other* is a layer no existing
P2 document offers.
...
3. **Working P2 dev** — can write a cog; uses this guide to fill in the parts of the architecture they skipped
   and to read any example fluently, then is **pointed OUT to the design book** (*The P2 Architect's Guide*)
   when they want the decomposition discipline.
4. **AI code-generating agent** — served the same orientation via the KB YAML (dual-rendering, §7).

## 5. The altitude arc — chapter architecture (3 chapters; a GUIDED ASCENT)
**Pedagogical spine (Stephen, 2026-06-22):** comfort first. Less-experienced engineers must feel at home with
the P2. So all three chapters are concrete and welcoming — picture the chip, learn to *read* its code, then
*use* the features. (Design reasoning — functional decomposition, the spatial-computing thesis — is the
sibling **design book**'s job; this guide links OUT to it.)

**Ch2 added (Stephen, 2026-06-23):** the original plan assumed the reader could already read a
...
|----|-----------------|----------|---------|-----------|
| 1 | **Meet the Propeller 2** | the territory, concretely | warm, feature-first mental model: the parts you can picture — 8 cogs, pins, hub, smart pins, CORDIC, streamer/FIFO, events, memory/boot, clock — and what each *does*. Build intuition through features, NOT abstraction. (Quietly seed one idea: "each cog just keeps running, independently.") *Orient, don't spec.* | the architecture spec, datasheet, the topic manuals |
...
Front matter: the thesis + how to read (audiences/paths). Back matter: a **"Where to Next"** map into the
reference manuals and OUT to the sibling design book (*The P2 Architect's Guide*) for system design.
(P1→P2 migration is woven as sidebars, not an appendix — §8.)

> **Reading paths** (one slim book, four audiences): newcomer = 1→2, then 3 when ready; P1 vet = follow the
> "P1 note:" sidebars through 1, then 3; working dev = straight to 3, 1–2 as reference, then OUT to the
> design book; AI agent = the same orientation via the KB YAML.

## 6. Source map (trust-chain grounding — nothing invented)
| Chapter | Primary sources (already in-repo) |
|---------|-----------------------------------|
| Ch1 | `deliverables/ai/P2/architecture/` — `p2-architecture-mental-model.yaml` (the AI-facing half, **already written**), `cog.yaml`, `hub.yaml`, `cordic.yaml`, `streamer/`, `event_system.yaml`, `interrupts.yaml`, `clock_system.yaml`, `boot-rom/`, `locks.yaml`, `lookup_ram.yaml`, `fifo.yaml`, `xbyte_engine.yaml`; the architecture spec; P2 datasheet |
| Ch2 | `guides/spin2-getting-started.yaml`, `guides/pasm2-getting-started.yaml`; `serial_loader.yaml` / `boot-rom/`; `language/` YAML |
| Ch3 | `language/spin2/` + `language/pasm2/` (cog launch, pin I/O, object/run-time model, hub sharing); `guides/spin2-getting-started.yaml`, `guides/pasm2-getting-started.yaml`; Smart Pins / Streamer guides (link-out targets) |
...
## 7. Dual-target rendering — RESOLVED (D4)
...
- **YAML:** **lightweight, granular, on-demand files** the download-on-demand system fetches on a single
  thread, so a consuming agent doesn't congest its conversational context. (The Ch1 mental-model + getting-
  started YAMLs largely exist too.)
- **Relationship:** siblings from one source understanding, each idiomatic to its medium — NOT mechanically
  synced identical prose. The YAML stays the agent-facing canonical form; the manual is the human face.
  Authoring keeps them *conceptually* in lockstep without forcing prose parity.

## 8. P1→P2 migration thread — woven sidebars (D3)
...
## 9. Voice & tone (the "how to voice it" half)

### 9.1 Positioning — where we post on the spectrum
...
### 9.2 The voice stays uniformly warm (the core move)
- **All three chapters (comfort):** maximally warm, reassuring, encouraging throughout. Assume intelligence,
  not P2 experience. "Here's the intuition; here's why it's nice." The job is to make a reader feel *at home*
  with the P2 — picture it, read its code, put it to work. Warmth is uniform: high warmth, LOW persona, start
  to finish. No "buckle up, it gets hard" rise — this is an on-ramp, not a climb.
- The guide **earns the trust** that lets the reader move on to the reference manuals and the design book.
  Never condescend; never turn glib.

### 9.3 Standing rules
- **Terminology:** "cog" not "CPU" (community treats the cog as the computer; lowercase in prose); canonical
  P2 terms; show code *constants* not arithmetic values; instruction/bit-field formatting per platform
  standards. Inherit the shared `repo-voice-profile.md` + platform voice; this guide layers the orientation
  register on top.
...
## 10. Design decisions — RESOLVED (Stephen, 2026-06-22; split 2026-06-24)
- **D1 — Name:** ✅ *Getting Started with the Propeller 2.* Plainly welcoming, searchable, says exactly what it
  is — the on-ramp. (The advanced "Architect's Guide" branding went with the design book at the split.)
- **D2 — Three chapters:** Ch1 picture the chip · **Ch2 read the code** · Ch3 put it to work. All three are the
  concrete, comfort-building orientation (§5); design reasoning lives in the sibling design book, linked OUT.
  Ch2 "Reading P2 Code" exists because the audience includes readers with no P1/Spin2/PASM2 background who must
  be shown the language *structure* before any example will read (its DoD is in §5).
...
- **D4 — Dual-target, reshaped for YAML:** ✅ lightweight, granular, on-demand YAML + warm human manual; same
  understanding, two shapes; not a 1:1 mirror (§7).
- **D5 — No length cap:** ✅ content sets length; brief but useful; link-out keeps it slim.

## 11. Risks & quality gates
- **Scope creep into reference territory** — the link-out contract (§2) is the guard; review every section
  for "am I duplicating Spin2 v55 / the Smart Pins manual?"
- **Scope creep into design territory** — system design / decomposition belongs to the sibling design book;
  if a section starts *prescribing how to architect a system*, link OUT instead.
- **Staying slim** — the no-length-cap discipline (D5) plus link-out is the feature; resist completeness.
- **Hallucination prevention** — same content-verification protocol as other manuals' creation-guides; every
  claim traces to the KB / architecture spec / datasheet.
...
## 13. Next steps
1. ✅ D1–D5 resolved (§10), pedagogy spine + uniform-warm voice locked; split from the design book (2026-06-24).
2. **NEXT:** draft the formal `creation-guide.md` + `voice-guide.md` from this charter (house format) —
   creation-guide carries the chapter specs + source map + verification protocol; voice-guide carries §9
   (positioning, the uniform warm-but-not-glib rules).
3. Then opus-master authoring Ch1→Ch3, each verified against its source-map, plus the "Where to Next" back-
   matter map (into the reference manuals and OUT to the design book).

## 14. Future / candidate additions (PARKED — not up next, don't forget)
...
- **CANDIDATE CHAPTER — "Using {toolchain}"** (the modern analog of v1.0's "Using the Propeller Tool").
  ...
  count if added (we're at three). **Not up next.**
