...
| 12 | G3 | Ch3 §3.3 Box Formula at boundary | **✅ FIXED v23 — keep-together standard.** Formula lengthened to a 10-line worked calc + forcing strengthened (`\leavebottom{2.2in}`). FormulaBlock made **non-breakable** (was `breakable`): the tall formula moves WHOLE to the next page rather than splitting its markerless left bar. Reference basis: keep short notation together, don't signpost a split. Verified v23 render: whole box on p21, no split, no margin overrun. GREEN (v24). | FIXED → green | keep-together | Resolves the old "too short / can't tell" note. |
| 12b | G3 | Ch3 §3.4 Syntax form at boundary (**NEW case**) | **✅ FIXED v23** — SyntaxBlock made **non-breakable** (same keep-together standard as §3.3). The multi-line `pasm-syntax` form forced to the foot moves WHOLE, never splitting its slate bar. Verified v23 render: whole box on p23 (`{#}D,{#}S` forms intact). GREEN (v24). | FIXED → green | keep-together | New case added + verified. |
| 12c | G3 | Ch3 §3.5 Register-layout diagram spans page (**NEW case**) | **✅ FIXED v23 — gap found in review.** LayoutBlock has the same full frame+fill as the code boxes and is `breakable`, but carried **no continuation markers** — a spanning bit-field diagram split silently (two bronze bands, ambiguous). Added `contmarkers` to LayoutBlock (shared C9/C10/C11 mechanism). New `layout` case forces a long diagram to span. Verified v23 render: footer "continues on next page →" on p25, header "↪ continued from previous page" on p26, bronze frame/fill intact both parts. GREEN (v24). | FIXED → green | continuation | Extends the shared continuation-marker standard to LayoutBlock. |
