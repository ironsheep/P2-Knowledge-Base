...
| 7 | G2 | Ch2 §2.1 Listing splits across page (C10) | Listing splits, but **no continuation marks**. Desired (user + C10): a "continued" line BELOW the listing on pg 1, and a "continued from previous page" block ABOVE the listing on pg 2. | DEFECT-BASELINE | — | **C10 continuation-marker standard not implemented** — primary Phase-1 fix target. |
| 8 | G2 | Ch2 §2.2 Long code lines | Long lines overflow the box — the word "well" is split and the next line runs right to the block edge; no clean wrap/handling. | DEFECT-BASELINE | — | long-code-line handling (wrap / scale / overflow rule) not implemented — fix target. |
...
| 13 | G4 | Ch4 Widows/Orphans in Prose | v6 split cleanly **BUT the EXPECT note says the foundation sets no `\clubpenalty`/`\widowpenalty` — so the clean break was COINCIDENTAL, not protection.** | DEFECT-BASELINE | widow-orphan | add `\clubpenalty`/`\widowpenalty` to the foundation. **Lesson: visual ≠ verified — the EXPECT note is the truth.** |
...
| 15 | G5 | Part II opener → Ch5 heading | Part-II divider and the Ch5 heading **overlap** (text overlaying). User: "expected — a good defect." | DEFECT-BASELINE | part-flow | part-intro→chapter-heading collision, correctly demonstrated. Real fix target. |
| 16 | G5 | Ch5 §5.1 Long table (60+ rows) | Long table **does not split** — runs off the page bottom, **nothing on the next page**. Also pushed itself whole onto the next page first (user: prefer it start on the current page and split). EXPECT: breaks across pages, repeating header. | DEFECT-BASELINE | — | long-table page-breaking + header-repeat not working — fix target. |
...
| 20 | G6 | Ch6 §6.1 Many Wide Columns | Wide-column table **overlap** demonstrated correctly. | DEFECT-BASELINE | table-fit | wide tables overflow/overlap — fix target (governed by the symbol-name constraint, #23). |
| 21 | G6 | Ch6 §6.2 Long Unbreakable Tokens (narrow col) | Long symbol tokens (unbreakable — correctly) cause **deep column overlap / overflow**. | DEFECT-BASELINE | table-fit | solve via **font-size reduction / column strategy — NOT token splitting** (#23). |
...
| 24 | G7 | Ch7 §7.1 Callout/diagram splits across page | Tall callout **splits to the next page** (good), but **no continuation signposting** — no "split/continued" at the foot of pg 1, no "continued from previous page" at the top of pg 2. | DEFECT-BASELINE | continuation | same family as 2.1 (C9/C10/C11 signposting) — applies to callouts/figures too. |
...
| 27 | G8 | Ch8 §8.1 Long callout spans page (C11) | Callout **splits across pages** (correct), but **no continuation signposting** (C11) — no split marker at the foot, no "continued from previous page" at the top. Also the EXPECT box + callout were both **pushed to the next page** (box separated from the 8.1 heading). | DEFECT-BASELINE | continuation | C11 signposting missing — same fix family as 2.1/7.1. (Box separation → #29.) |
