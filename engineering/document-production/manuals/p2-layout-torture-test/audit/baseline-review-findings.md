...
| 8 | G2 | Ch2 §2.2 Long code lines | **✅ FIXED v16** — colored code blocks now load `fvextra` and set `breaklines` on the Verbatim, so over-wide lines wrap inside the box, each continuation prefixed with a hook (↪) and auto-indented. Breaks land at spaces (`breakanywhere` off) so mnemonics/identifiers stay whole. Verified p14: both long-comment lines wrap inside the green box, nothing past the right edge. GREEN. | FIXED → green | long-line | **Phase-2 fix #4 complete.** Applied data-set-wide to all five colored-block environments (Spin2/IOSP/CORDIC/MultiCOG/Antipattern). |
