...
# Skip backup files and other non-source files
SKIP_PATTERNS = re.compile(r"\.(backup|bak)(\.|$)")

# Match a fenced code block opener: ```pasm2 or ```spin2
...
def extract_blocks(md_path: Path):
    ...
    while i < n:
        ...
        if not m:
            ...
            continue
        lang = m.group(1)
        start = i + 1  # 1-based line number of the opening fence
        j = i + 1
        body = []
        while j < n and not FENCE_CLOSE.match(lines[j]):
            body.append(lines[j].rstrip("\n"))
            j += 1
        end = j + 1  # closing fence line number
