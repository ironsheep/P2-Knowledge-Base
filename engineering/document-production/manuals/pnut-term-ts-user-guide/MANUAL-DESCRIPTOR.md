...
fragile_areas:
  ...
  - "AGENT PLACEMENT GATE — nothing about AI agents, AI coding assistants, or P2KB MCP may appear before Part 3 (Headless and Automation). Parts 1-2 teach a PERSON to operate the tool. Check: grep the body for agent|assistant|AI |MCP|P2KB|Architect and confirm every hit is at or after the 'Part 3: Headless and Automation' heading (Ch19 Support and Resources is also past the gate and may cite Architect Part 3). RATIONALE, not style: the tool-chain teaching 'the loop closes through the log file' is TRUE for an agent and FALSE for a person at the GUI, whose return path is the screen — so an early mention teaches the wrong loop and demotes all of Part 2 to a side effect. The Ch1 and Ch15 figures are a deliberate matched pair on one spine (same nodes, different return leg); if either is edited, keep them structurally parallel. See creation-guide.md §0."
---
