...
| `pipelines/document-production-pipeline.md` | per-document deliverable state | ✅ **BANNERED 2026-09-10** — superseded by `PUBLICATION-ROSTER.md`. Not deleted: four live documents still link to it, so it carries a do-not-plan-from-this banner naming what it gets wrong (Smart Pins Reference retired, DeSilva shown *Queued* at v3.0.6 published, Single-Step Debugger shown *Content Ready* and released, and three working paths that no longer exist). Retiring the four referrers is still owed. |
...
| `operations/README.md` | the ops dashboard | ✅ **DONE 2026-09-10** — thinned to a pointer exactly as prescribed here. The V1.0–V1.3 milestone board it had become (last true 2025-09-13) was retired to the **gitignored** `operations/archive/` under a banner — local for retrospectives, with `git show 6766f4b7:engineering/operations/README.md` as the durable record. The live file now points at the registers, the process map and lessons-learned; all 13 of its links resolve. `operations/INDEX.md` went the same way — an orphan with **53 of 53 links dead**. |
