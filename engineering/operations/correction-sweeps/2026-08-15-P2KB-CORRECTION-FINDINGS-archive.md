# P2KB Correction Findings — ARCHIVE, sweep of 2026-08-15

Findings closed between **F-125 and F-266** — `DONE`, `RESOLVED-INVALID`, `FIXED`, or applied-and-verified.
Swept out of the active register on 2026-08-15 per `punch-list-maintenance`, so that register carries
only work still owed. This file is **complete for what it holds and is never re-edited**; if an archived
finding must be reopened it returns to the active register as a *new* item referencing this file.

**Earlier archive:** F-001…F-124 → `2026-06-13-P2KB-CORRECTION-FINDINGS-archive.md`.
**Active register:** `engineering/operations/P2KB-CORRECTION-FINDINGS.md` — open findings only.
Never re-derive completion state from this file; the active register is the authority on what is outstanding.

**Search here before re-filing.** Most past defects, and the reasoning that settled them, are here.
A finding that reappears is usually a regression, not a new discovery.
