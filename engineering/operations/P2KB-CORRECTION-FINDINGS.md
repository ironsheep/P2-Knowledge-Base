...
### G-001 — WRPIN `%AAAA`/`%BBBB` input-selector relative-pin sub-field is undocumented in our YAML — `CONFIRMED (add)`
...
### G-002 — smart-pin timing-mode YAMLs don't record the DIR=0 `Z` preload value — `CONFIRMED (add)`
...
### G-003 — DAC "16-bit" claims lack the nominal/averaged ENOB caveat — `CONFIRMED (add — framing only)`
...
### G-004 — `architecture/smart-pins/smart-pin-11011-usb-host-device.yaml` X/Y/Z registers are one-line stubs — `PARTIAL (Silicon now · remainder Chip-gated)`
...
### G-005 — `architecture/smart-pins/smart-pin-11110.yaml` (async TX) omits the first-byte low-glitch gotcha — `NEEDS-HW-CONFIRM (record as attributed gotcha)`
