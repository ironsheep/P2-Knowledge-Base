...
- **Empirical arm (run 1 = 2026-07-13, INCONCLUSIVE → rig revised, re-run pending):** `vt2-mailbox-publish-order.spin2` arm **C (`NOACK`)**. Run 1: arm A (R3 as printed) = **0 bad**, arm B (bad publish order) = **20,000 bad**, arm C = **0 bad** → the rig self-reported INCONCLUSIVE rather than a false pass. Cause was the **rig, not the recipe**: rev-1's worker spun tight and began reading the instant `cmdSeq` moved, finishing all three reads before the writer's next write could land. **Rev 2** adds a `WORKER_GAP_US` between reading the opcode and reading the args (modelling the real `CASE cmd.opcode` dispatch), applied identically in all three arms — in A/B the ack blocks the writer through the gap, in C it does not. Re-run pending; promote to the EF ledger once it certifies. **The fix to the note does not depend on this measurement** — it stands on the correctness argument above, which run 1 already sharpened.
