#!/usr/bin/env python3
"""
Convert PASM2 instructions to UPPERCASE (no bold markers) in markdown code blocks.
For LaTeX/PDF generation, the template's lstset will handle the bolding.
"""

import re
import sys

# Common PASM2 instructions to convert to UPPERCASE
INSTRUCTIONS = {
...
def process_line(line, in_code_block):
    """Process a single line, converting instructions to UPPERCASE (no bold) if in code block."""
    ...
    if words:
        for i, word in enumerate(words):
            ...
            if word_lower in INSTRUCTIONS:
                # Replace with UPPERCASE only (no bold markers)
                words[i] = word.upper()
...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        print("Usage: python uppercase-instructions-latex.py input.md output.md")
        sys.exit(1)
