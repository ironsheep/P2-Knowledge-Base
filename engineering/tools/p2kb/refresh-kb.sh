...
# Use subshell to isolate from any shell hooks, head -1 to handle any doubled output
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd -P | head -1 | tr -d '\r')"
...
# Fetch script (with cache-control header to bypass GitHub CDN cache)
log "Downloading fetch-kb-file.sh"
curl -sS -H "Cache-Control: no-cache" "$BASE_URL/engineering/tools/p2kb/fetch-kb-file.sh" > "$SCRIPT_DIR/fetch-kb-file.sh.tmp"
...
curl -sS -H "Cache-Control: no-cache" "$BASE_URL/engineering/tools/p2kb/refresh-kb.sh" > "$SCRIPT_DIR/refresh-kb.sh.tmp"
...
curl -sS -H "Cache-Control: no-cache" "$BASE_URL/engineering/tools/p2kb/obex/download-helper.sh" > "$SCRIPT_DIR/obex/download-helper.sh.tmp"
...
curl -sS -H "Cache-Control: no-cache" "$BASE_URL/deliverables/ai/p2kb-index.json.gz" > "$INDEX_FILE.tmp.gz"
...
for key in "${COMMON_KEYS[@]}"; do
    ...
    if [[ -n "$path" ]]; then
        ...
        curl -sS -H "Cache-Control: no-cache" "$BASE_URL/$path" | filter_metadata > "$cache_file.tmp"
...
curl -sS -H "Cache-Control: no-cache" "$BASE_URL/manifests/propeller-knowledge-root.yaml" > "$INDEX_DIR/propeller-knowledge-root.yaml"
status "propeller-knowledge-root.yaml"

log "Downloading ai-instructions.yaml"
curl -sS -H "Cache-Control: no-cache" "$BASE_URL/manifests/ai-instructions.yaml" > "$INDEX_DIR/ai-instructions.yaml"
