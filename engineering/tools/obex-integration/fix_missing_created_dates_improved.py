#!/usr/bin/env python3
"""
Fix missing created_date fields with improved patterns including Object ID (date) format
"""
...
class CreatedDateFixer:
    ...
    def extract_created_date_from_obex(self, obex_url, obj_id):
        """Extract created_date from OBEX page with improved patterns"""
        if not obex_url:
            return None
        ...
        try:
            ...
            # Look for creation date patterns - prioritizing Object ID format
            date_patterns = [
                rf'Object\s+ID\s*:\s*{obj_id}\s*\((\d{{4}}-\d{{2}}-\d{{2}})\)',  # Object ID : 2811 (2020-05-09)
                r'Object\s+ID\s*:\s*\d+\s*\((\d{4}-\d{2}-\d{2})\)',  # Any Object ID with date
                r'\((\d{4}-\d{2}-\d{2})\)',  # Any date in parentheses
    ...
    def fix_missing_dates(self):
        ...
        print("Using improved patterns including Object ID (date) format...")
        ...
        for i, missing in enumerate(self.missing_dates, 1):
            ...
            created_date = self.extract_created_date_from_obex(missing['obex_url'], missing['object_id'])
        ...
        print(f"\n✅ IMPROVED CREATED DATE FIX COMPLETE:")
...
def main():
    ...
    print("IMPROVED CREATED DATE EXTRACTION")
    print("=" * 70)
    ...
    if missing:
        print(f"\n🔧 FIXING REMAINING MISSING CREATED DATES:")
        print("-" * 50)
        
        print(f"\nUsing improved patterns to extract from {len(missing)} OBEX pages...")
