...
| p2-eval-board (#64000 Rev C) | **2026-08-24 re-extraction** (`sources/p2-eval-board/`) | 2025-08-29/09-02 PDF-era capture (`-narrative.txt` + audit + cross-source, shipped at `100% (stated)`) | **re-extraction — F-250.** The prior capture was not merely incomplete, it was **wrong in a way that read as complete**: the PDF's body font maps no numerals, so `pdftotext` returned fluent prose with **every digit deleted** (10.0% of non-blank lines carried a digit vs a 29–58% peer band on that metric; **1.5%** on the gate tool's ≥12-char metric). Repaired with `pdf-ocr --force-ocr` → **52.6%** non-blank / **56.5%** tool; both ruled tables re-cut with `camelot lattice`; **all 17 pages read against the rendered page**; edge-header pin order recovered from figures (it is in no text layer). Prior archived `archive/` + pointer README (§0.6), together with a fresh `pdftotext` run kept as the evidence exhibit. **Downstream re-validation:** extraction audit + cross-source analysis corrected in place; `hardware/p2-eval-board.yaml` conflicts routed to **F-328**. General lesson made executable: `tools/validation/audit-extraction-digit-density.py`, wired into `ingest-source` §2a. | prior archived (not deleted) |
| p2-hardware-manual | **2026-08-24 DOCX-primary re-ingestion** (`sources/p2-hardware-manual/`) | 2025-08-15 analysis summaries (`hardware-manual-2022-extraction.md` + audit) | **re-extraction** — the prior effort committed **no extracted text at all**, only summaries, while its audit read *"Extraction Health 100%"* and the dashboard read ~65%. Now 3,198/3,198 paragraphs, **53 tables** (incl. 4 nested-in-cell), 117/117 TOC entries, 37-figure image catalog built from nothing, 24 `pnut-ts`-validated fragments. **Two DOCX copies reconciled by measurement: same edition, exported twice — content byte-identical**; canonical = the `sources/` copy. | **Kept, marked in place** — `hardware-manual-2022-extraction.md` carries a SUPERSEDED pointer rather than being moved, because `YAML-CONSTANT-FIDELITY-SPRINT-PLAN.md` §8 and `p2-eval-board-cross-source-analysis.md` reference the path (§0.6 Titus refinement: redirect first, move after). |
...
| `hardware/hub75_adapter.yaml` | **p2-hub75-adapter** (#64032) — the three authored captures in `sources/p2-hub75-adapter/`; the file cites them by `path:line`. **No vendor primary document exists to stand behind them** (the board has no Parallax datasheet or user guide), so the captures ARE the source of record. | `-preliminary-knowledge.md` is ⚠️ Draft/Partial by its own wording — corroborate before use. The #64032 rows in `extraction-matrices/p2-edge-complete-ecosystem-compatibility-matrix.md` and `sources/p2-board-addon-compatibility-matrix.md` derive from these same captures and are **not** an independent check. |
...
|----------|----------|----------|-----------|------------------------|
| **pasm2-manual PDF-derived narrative (2025-08)** | **augment + re-extraction** | pasm2-manual DOCX-primary (2026-08-26) | The DOCX existed in `external-inputs/p2/` and was never staged, so the 2025 audit recorded a `.docx` extraction the source folder could not support (→ **F-367**). DOCX now staged alongside the PDF. 219/219 tables where the narrative had none; digit density 61.3% vs 50.4%; more words in fewer characters. `pasm2-manual-narrative.txt` retained in place for lineage (deprecated narrative form, not deleted). | **pasm2-manual DOCX 2026-08-26** |
| **taqoz web research (preliminary)** | **superseded by authoritative source** | TAQOZ Bit Bashers Guide DOCX (2026-08-26) | `sources/taqoz/taqoz-web-research-preliminary.md` is unverified web research; the Bit Bashers Guide is an authored guide by the language's creator. The dashboard row already anticipated this, carrying **🟡→🏆** with *"authoritative .docx now staged"*. **Two folders, one logical source** — the folder merge is a registry decision and was deliberately NOT taken during ingestion. | **TAQOZ Bit Bashers Guide 2026-08-26** |
| edge-standard-module (#P2-EC Rev D) | **completion, no supersession** | — | The 2025 extraction was not replaced; a `-text.txt`, 4 camelot tables, a 6-image catalog and the **missing audit of record** were added to it. Derived fact recorded: **SD-card boot selects on P59** (△/▽ = P59 up/down), which closed **G-026** and corrected the P60 mechanism proposed in Silicon Doc reviewer comment [25]. | edge-standard-module Rev D v3.0 |
| p2-hub75-adapter (#64032) | **registered, not ingested** | — | The folder was created 2026-08-26 («#322») from two loose captures sitting in the `sources/` root, and the source then appeared in **neither** this file nor `AUTHORITATIVE-SOURCES.md` until it was registered 2026-08-27. There is no ingestion pass behind it — no `-text.txt`, no extraction audit, no image catalog, no pass-6 cross-source work — **because there is no vendor document to extract**: the board ships without a Parallax datasheet or user guide. Meanwhile `deliverables/ai/P2/hardware/hub75_adapter.yaml` was already citing the captures by `path:line`, so an unregistered source was load-bearing in the shipped KB. | the three authored captures themselves (🟢, Iron Sheep Productions = the manufacturer). A real ingestion becomes possible only if a vendor primary appears; until then the registration IS the record. |
| p2-click-adapter (#64008) | **new source** | — | First ingestion. Schematic-only board — no Parallax user guide exists. Feeds `hardware/addon-click-adapter.yaml` (new) and re-sources `architecture/click_module_integration.yaml`, whose 12 pin offsets had been `documentation_source: code_analysis` and were confirmed exactly, 0 corrections. | Rev A (2020-08-10) |
| **silicon-doc PDF-era capture (2025-08)** | **re-extraction (superseded)** | silicon-doc DOCX-primary (2026-08-26) | The 2025 capture is PDF-derived and carries **zero of the document's 48 tables**; it also splits code lines (`p2-documentation.txt:208` strands a comment whose `SETQ #16-1` was torn away). It produced at least one wrong fact that reached shipped YAML — **F-363**, the `$1F6`/`$1F7` PA/PB row. **MARK-IN-PLACE, NOT MOVED:** 293 references to `p2-documentation.txt` exist outside the folder (24 in the shipped KB, 23 with `:line` locators), so an archive-move would strand every one (`ingest-source` §0.6; Sacred Rule #7). Marker: `sources/silicon-doc/SUPERSEDED-BY-2026-08-26-DOCX.md`. Re-anchoring tracked as **F-365**. The 2025-08-15 audit of record is preserved verbatim at `sources/silicon-doc/archive/silicon-doc-complete-extraction-audit-2025-08-15.md`. | **silicon-doc DOCX-primary 2026-08-26** |
...
| p2-hardware-manual | **corroborates** | p2-datasheet | nominal system clock **180 MHz @ 105 °C** (verbatim in both) · core/IO supply 1.8 V / 3.3 V · **max current per I/O ±30 mA** · internal osc ~24 MHz / ~20 kHz · XI-XO capacitance 15 pF / 30 pF | agree (HIGH) — the datasheet adds min/typ/max, the manual states typ. 2026-08-24 |
| p2-hardware-manual | **vocabulary key for** | p2-datasheet | crystal-capacitance modes: manual says `%10` / `%11`, datasheet says "Mode 2" / "Mode 3" — **same values, different naming** | not a conflict; recorded so it is not re-filed as one (cf. the microSD `DI/DO ≡ MOSI/MISO` resolution) |
| p2-hardware-manual | **repairs structurally-lost tables in** | p2-datasheet | `identified-broken-tables.md` #2 Pin Descriptions · #3 Smart Pin Mode Config (the datasheet extract left only `OUT / CIOHHHLLL / DIR`) · #6 I/O Pin Equivalent Circuit · #8 Clock Mode Settings | **the DOCX carries table structure a PDF text layer cannot** — this is what makes the manual a usable cross-check partner for plan §8 |
| p2-hardware-manual · p2-datasheet | **both silent** | `deliverables/ai/P2/architecture/io_pin_timing.yaml` | I/O pin propagation delay / rise time in **nanoseconds** | **neither source states ns** — the manual gives clock counts only (3/3/2) and its timing figures are clock-numbered; the datasheet AC table has only `Freq` and `Cin`. → **F-329**. The manual **does** corroborate the file's clock-cycle half exactly. |
