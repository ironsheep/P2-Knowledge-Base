# P2 Documentation v35 — Rev B/C Silicon — curated extraction
...
So tell me what those 3 instructions do.  I know the next line does that, but give me a high level description.
xThe idea is that S/# can serve as a register base address and D can be used as an index. The idea is that S/# can serve as a register base address and D can be used as an index.
These instructions sum their D[8:0] and S/#[8:0] values to compute an address that is directly substituted into the next instruction's S field, D field, or result register address (normally, this is the same as the D field).
So when is D and R different fields?
This all happens within the pipeline and does not affect the actual program code.
Additionally, S[17:9] is always sign-extended
What does sign extended mean?  And I thought you were using S[8:0] You lost me here. Again a high level description please.
and added to the D register for index updating. Normally, a nine-bit #address will be used for S, causing S[17:9] to be zero, so that D is unaffected:
```
...
```
```
So this is a good place to say when you would use altr vs altd
```
```
...
```
```
I think I now get it.  My point is that you could make it so much easier for me as the reader to udnerstand it, if you
```
```
Assume I know nothing about your assembly langauge, and introduce one concept a ttime.
```
...
Why 13:5? It looks odd.  A word of explanation is in order.  Why not 8:0  In general an inroduction to odd offsets into words is claled for.  I see you you a lot of such odd offsets below.
And earlier you said registers are accessed at 32 bit addresses, now you are correcting that.  You should mention up front that registers can be accessed as nibble/byte/long/word, but address pointers are at the 32 bit word level.
Also why not access it at the long level??? It seems you only do nibble/byte/word.
...
```
Whoa.   This tell sme the details of the alti, but gives me no context to understand its motivation.  Just comes across as stange.  Furthermore I have no idea how or when to use it.  Start with some motivation please.
For more complex S field, D field, and result register substitutions, there is the ALTI instruction. ALTI actually does a few different things. First, ALTI can be used to individually increment or decrement three different nine-bit fields within a register.
By now I kind of figured out that S and D are fields in a register.  Maybe they are registers themselves.  So some introduction should say more about the 3 fields.  What is the third one?  What are the remaining bits used for? 9 bits mean they cover register and lutram, but most instructions do not operate on lutram.
Second, ALTI can substitute each of those fields (before incrementing or decrementing) into the next instruction's S field, D field, or result register address, in the same way ALTS, ALTD, and ALTR do. Lastly, ALTI can substitute D[31..18] into the next instruction's upper bits [31..18] to enable
The next line  seems like an odd thing to do.  When would I want to do that?
full instruction substitution with a register's contents.
```
...
I/O pins are read by cogs via the following cog regis+ters:
...
Once a smart pin is configured via WRPIN and then started by making its DIR bit high, it can be reset at any time by making its DIR bit low. It does not lose its configuration set by the last WRPIN, although some mode-related state bits will be reset while DIR is low. Once DIR is returned high, it resumes operation in the same mode as previously configured, but with some reset state data. You can think about this by considering that each smart pin has 126 bits of state data, aside from its WRPIN configuration data. The configuration set by WRPIN simply multiplexes those 126 bits to the unique smart-pin subcircuit that was selected by the five %SSSSS mode bits. You could do a WRPIN while DIR is high, but the unregulated change in use of the 126 state bits would cause unpredictable and quite certainly useless behavior in the newly-selected smart pin mode. This is why smart pin modes should be selected and configured only while DIR is low.
...
Y.[15..0] establishes the DAC output value which gets captured at each sample period and used for its duration.
On completion of each sample period, Y.[15..0] is captured for the next output value and IN is raised. Therefore, you would coordinate updating Y.[15..0] with IN going high.
PWM dithering will give better dynamic range than pseudo-random dithering, since a maximum of only two adjacent 8-bit DAC levels are set for every 256 clocks. This means, though, that a frequency of Fclock/256 will be present in the output at -48dB.
If OUT is high, the ADC will be enabled and RDPIN/RQPIN can be used to retrieve the 16-bit ADC accumulation from the last sample period. This can be used to measure loading on the DAC pin.
During reset (DIR=0), IN is low and Y.[15..0] is captured.
