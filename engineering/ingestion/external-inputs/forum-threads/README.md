...
| S3 | HDMI audio (data-island) needs ~34 hblank pixel-periods vs ~16 video-only | HDMI audio | ❌ **RETRACTED** — Wuerfel_21 later corrected 34 as ~40% low; relay-by-cgracey ≠ endorsement, never spec-sourced. Figure **removed in v1.0.4**; §15.2 now points to the HDMI data-island spec. |
...
**Status (2026-07-01) — RECONCILED into the Streamer Programming Guide; shipped in v1.0.3 (2026-07-03).** S1 folded as a §10.4 caution (SINC2 constant-iteration-count silicon limitation + 3 workarounds; Silicon-Doc note 2024.12.16) + an App-D checklist item. S2 + S3 folded into §15.2 (blanking display-limited 16–68 px / mult-of-8 / ~8-line vblank; audio ~34 px flagged community-measured, verify vs data-island spec).

**CORRECTION (2026-07-04, shipped v1.0.4) — forum-provenance sweep.** Two of the S2/S3 figures were forum-tier numbers stated above their source's confidence and were removed/reframed per the Titus-tier discipline (`feedback_forum_material_is_titus_tier`; `document-audit` #4b / `ingest-source` §5): **S3 (~34 px audio)** — RETRACTED by its author (Wuerfel_21: ~40% low), relayed-not-endorsed by cgracey, never spec-sourced → removed; §15.2 now points to the HDMI data-island spec. **S2 "multiple-of-8"** — that rule is documentary for the `hdmi_base` *pin number* (Silicon Doc), NOT for blanking width → the blanking-width "must be mult-of-8" assertion removed. The genuinely-observed per-display floors (16–68 px, ~8-line vblank) stay, framed as display-specific observations. S1's "< 20 ms" bound reframed as approximate/verify (Chip's forum figure, not in the Silicon-Doc note). S5 folded as a §9.2 capture-to-spectrum pointer (→ P2AN002). **S4 DEFERRED** — "SETQ block RDLONG/WRLONG = 1 clk/long" is a hub-transfer fact, a *different* mechanism from the streamer's RDFAST/WRFAST FIFO; folding it into the FIFO discussion would be a technical conflation. Route to PASM2/hub docs, not this guide. **Y5 (SETPIV/BLNPIX)** → PASM2 YAML (yaml head), ⚠️ community-reverse-engineered — verify semantics before use. **D1 (8-bit sub-pixel model)** → DEBUG-window manual, not the Streamer guide.
