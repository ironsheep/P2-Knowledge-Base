...
| **VO-J-004** | Two questions one run can settle: (a) does the **debug interrupt in the measuring cog** change EF-062/EF-063's answers? (b) where does the streamer's **pin group** actually land — the idiom «#289» is about to author into five blocks of a released manual | Same P0→P1 jumper. **Phase 1** runs the identical EF-062/EF-063 measurement twice — leg A in cog 0 with debug live (reproducing run 3) and leg B in a launched, debug-free cog — and compares: digital legs exactly, analog legs as verdicts. **Phase 2** builds a *canvas* — P8..P31 at a 15 kΩ low with DIR high, so an undriven pin reads 0 for a stated reason and any driven pin overpowers the pull and reads 1, making the readback a map of which pins were driven; it certifies every pin free *weakly* before driving anything hard, and names them and stops if not. Four tests against it: **B** `drvh` span 16..23 with no streamer (**the control for the rest**), **C** 8-pin `X_IMM_4X8_1DAC8` at aligned base 16, **D** 1-pin `X_IMM_32X1_1DAC1` at pin 20, **E** the 8-pin mode at base 20 composed with **`+`**, the manual's own idiom. Command words verified from the compiler listing first: `$60AE_FFFF`, `$40A8_FFFF`, `$60B6_FFFF`. Rig: `…/audit/verification-tests/test-f308-cog-and-pingroup.spin2`. | (a) retires the `[M — single run]` caveat on EF-062/EF-063, or regrades both if the cogs disagree. (b) settles the pin-field question the Streamer Guide and the Silicon Doc appear to state differently — **they don't disagree, the field is mode-dependent**: `X_IMM_4X8_1DAC8` prints `D[19:16]` as a literal `%1110` (no pin field, group from `D[22:20]` in 8-pin steps) while `X_IMM_32X1_1DAC1` prints `%pppa` (any pin, from `D[22:17]`). The first draft of test D was **vacuous** — with `\|`, `20<<17` sets a bit the mode template already sets, so the word came out byte-identical to base 16; the compiler listing caught that, not the bench. Test E is the live trap: with `+` the same unaligned base *carries* into `D[19:16]`, giving a different mode at a different group — EF-059's failure in a second mode family. | **DONE → EF-064 + EF-065**, and the EF-062/EF-063 caveats are **retired** (2026-08-20, first run, clean). Every prediction held. Phase 1: legA/legB matched on every digital count and agreed in verdict on the analog — the debug interrupt changed nothing, and both entries gained their confirming second run. Phase 2: canvas `$0000_0000` · B `$0000_FF00` · C `$0000_FF00` · D `$0000_1000` · **E `$00FF_0000` — the trap fired exactly as predicted**, → **F-309**. Two cosmetic authoring bugs in the rig, fixed, no re-run needed: a bare `"B"` is a Spin2 *character constant*, not a string pointer, so `zstr_` read from hub address 66 and emitted control characters; and a `«#289»` in a `debug()` string put non-ASCII bytes on the wire, which PNut-Term-TS correctly flagged as binary. Both are exactly what the **owed** `.spin2` style gate («#217», `STYLE_GATE_COMMAND` unset) exists to catch. |
| **VO-J-005** | Whether the streamer's **digital pin capture** path works at all, and the fact that decides how it is used: on a pin running a smart-pin mode `IN` is that smart pin's event flag, so capturing the bus pins records ready-pulses while capturing a **neighbour-routed non-smart pin** records the waveform. Grounds `architecture/smart_pins.yaml` `in_signal_semantics` and the new `architecture/streamer/pin-capture.yaml`, neither of which has any empirical backing today. | **Bare board — NO jumper, no instrument.** A TRANSITION-mode smart pin on P8 generates the stimulus; P9 and P10 take their `A` input from it via `P_MINUS1_A` / `P_MINUS2_A` (the second through `P_FILT0_AB`); P11 is driven low as a null lane. One `X_4P_4DAC1_WFBYTE | X_WRITE_ON` capture at base 8 records **all four lanes in the same buffer at the same instant**, so the contrast is one result rather than two runs compared. Command words verified from the compiler listing first ($E090_0000 at base 8, $E098_0000 at base 12). Arms: **A** four statically driven pins, twice, opposite ends high — the capture-path control and the lane calibration, and it gates the run; **B** a capture at base 12 to settle where an unaligned-for-8 base actually lands (F-361); **C** the same contrast read by the cog with no streamer involved, so the IN rule is isolated from the capture path; **D** the headline, with the burst armed to END mid-window so the smart pin's lane carries exactly ONE edge — a live lane carrying a different signal, not a dead lane; **E** an NCO ladder from 1:16 to 1:1 reporting measured vs predicted half-cycle width, which measures the sustained ceiling instead of asserting one. Every buffer is pre-filled with an `$A5` sentinel, so "captured nothing" is distinguishable from "captured zeros". Rig authored + compiled: `…/campaigns/2026-08-manual-corrections/tests/test-vo-j-005-streamer-pin-capture.spin2`. | Closes the gap that **every streamer finding we hold is output or ADC** — EF-054/055/059/062/063/064/065 have never put the Pins-to-WRFAST digital capture path on silicon. It also settles two things no source we hold states: the **sustained sample-rate ceiling** (arm E measures it; nothing in the KB claims one) and whether **`%FFF` filtering adds inter-channel skew** (arm D measures it as a lane-to-lane first-edge offset). And arm B independently checks the F-361 correction to `pin-selection.yaml`'s sub-pin table, with the reversing outcome named. | **AUTHORED, NOT RUN** (2026-08-26). Compiles clean under `pnut-ts -d` v1.55.3. Predictions and their falsifying outcomes are written into the program before the run, per campaign convention. |
| **VO-J-006** | **Paired-cog LUT write-sharing (`SETLUTS`) on silicon** — the mechanism the Silicon Doc states (`silicon-doc-text.txt:491-504`) and the P2 Datasheet repeats (`p2-datasheet-text.txt:610-616`), which the KB (`architecture/lookup_ram.yaml`), the Assembly manual §1.3.3 and the Architect's Guide all teach, and **which no EF entry has ever exercised**. Raised 2026-09-13 by the LUT app-note research (F-428). | **Bare board — NO jumper, no instrument.** Every observation is a cog reading its own LUT back, so no pin is involved (classification earned by that trace, not by appearance). Start a pair with `COGINIT #%1_1_0001` (`silicon-doc-text.txt:413`, `:502`) and report from a cog that is not measuring (EF-057). Arms, predictions written into the program first: **A** negative control — receiver at `SETLUTS #0` (the default), sender `WRLUT`s a sentinel; the receiver's LUT must be **unchanged** (a pass here means the rig cannot see sharing). **B** receiver `SETLUTS #1`; sender writes distinct values at `$000`, `$0FF`, `$100`, `$1FF`; all four must arrive. **C** direction — the receiver's own `WRLUT`s must **not** reach a sender that never enabled. **D** which writes count as "local lookup RAM writes": `WRLUT` versus a `SETQ2`+`RDLONG` block load into the sender's LUT. No source says whether the block path propagates. **E** the handshake the Silicon Doc names (`:504`): receiver arms `SETSE1 #%000_00_11AA` ("companion cog writes LUT address %1111111AA", `:2249`) and must see SE1 fire on a sender write to `$1FC`-`$1FF`, and must **not** see it on a write below `$1FC`. **F** Spin2 in the pair — the Spin2 interpreter occupies LUT `$010..$1FF` (`spin2-v55-text.txt:806`, `:834`): with a Spin2 cog as sender and a PASM cog at `SETLUTS #1`, does the interpreter's LUT content arrive in the receiver? No source addresses it either way; measure, do not assume. | First empirical grounding for a mechanism three shipped documents teach. **D** and **F** settle two questions **no source we hold answers**: which write paths propagate, and whether sharing is usable at all with a Spin2 cog in the pair. **F**'s answer decides whether LUT sharing can become a **P2AN007** cross-cog recipe (the Spin2 STRUCT app note), and the whole result feeds the punch-listed revisit of a stand-alone LUT app note (`document-production/PUNCH-LIST.md`). | **IDENTIFIED, NOT AUTHORED** (2026-09-13). |
