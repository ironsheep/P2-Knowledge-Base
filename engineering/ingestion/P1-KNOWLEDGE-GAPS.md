...
| G-P1-008 | P1 library / driver objects | The app-note + PE-Labs code reuses **stock Propeller-Tool / OBEX driver objects** not yet inventoried in the KB: `VGA_HiRes_Text_010`, `Keyboard_011`, `Mouse_011`, `WMF_Framework_010` (AN004 + AN013), `NS_sound_drv`, `pwmAsm_010` (AN013); AN001 references `VGA_Text.spin` + `CTR.spin`; **PE Labs** uses `Parallax Serial Terminal.spin`, `Float`, `FloatString` — all **not shipped** in their archives. Capture this shared P1 driver/library set as its own ingestion. | OPEN | P1 library-objects ingestion / Propeller Tool library / OBEX | 2026-06-27 | — |
