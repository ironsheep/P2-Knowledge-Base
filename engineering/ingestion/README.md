...
- **What's next:** **(1) HyperRAM/HyperFlash board (#64004-ES) ingestion** — staged, full 7-pass · **(2) silicon-doc re-extraction from the `.docx`** — clean image catalog (Titus-style) + modern re-validation; `.docx` confirmed in `external-inputs/p2/`. _**Titus DETUNED 2026-06-22** — superseded by the IOSP "Blue Book" manual (which carries our own generated figures); Titus stays 🟡 cross-check and its 17-figure OCR-label debt is **WON'T-DO**._  ‹Titus rev5 ✅ 2026-06-12 · YAML v1.10.1 sweep ✅ 2026-06-20 (G-001 WRPIN selector shipped)›
...
| **next (1)** ‹queued 2026-06-13› | 64004-ES HyperRAM/HyperFlash Memory Board (Parallax #64004-ES) | **SCHEDULED 2026-06-22.** Product Guide PDF **staged** `sources/hyperRam-n-hyperFlash/` (+ REVA schematic/PCB CAD). Fills a hardware coverage gap: **no standalone board YAML today** — only referenced as a compatible add-on in `deliverables/ai/P2/hardware/p2-eval-board.yaml`. 16MB HyperRAM + 32MB HyperFlash. Full 7-pass `ingest-source`; **do NOT fabricate from raw CAD** — author from the Product Guide. (Surfaced in the 2026-06-13 YAML corrections pass.) |
| **next (2)** ‹scheduled 2026-06-22› | silicon-doc — **re-extraction from the `.docx`** (v35 Rev B/C) | `.docx` confirmed: `external-inputs/p2/Parallax Propeller 2 Documentation v35 - Rev B_C Silicon.docx`. Prior 2025-08 docx text-extract (`sources/silicon-doc/silicon-from-docx.md`) already measured the win (**607 sections, 48 tables** vs PDF/text) but the **clean image catalog was never built** (the 2025-09 `IMAGE-ENHANCEMENT-PLAN`, never executed). Redo via the modern DOCX-primary `ingest-source` path: lossless `word/media/*` image extraction + `image-tools-mcp` catalog (Titus-style) + pnut_ts code re-validation + refreshed audit. **Crown-jewel source — highest downstream leverage; sequence after HyperRAM.** |
| **queued** ‹2026-06-13› | #64006 P2 Eval Add-on Boards — **cross-edition** (Aug-2020 `#64006-ES` + Aug-2025 `#64006`) | 2025 edition already ingested (`sources/p2-eval-add-on-boards/`); **stage the 2020 `#64006-ES` Product Guide** (user holds it) and cross-check both for authoritative **per-individual-board pin maps** (the bundle SKU doesn't matter to agents). Reconciles `hardware/addon-*.yaml`: resolves the 2 spurious entries (`addon-digital-io-board`, `addon-servo-header`) + the 4 part-number-less orphans. See corrections-register **F-121**. |
