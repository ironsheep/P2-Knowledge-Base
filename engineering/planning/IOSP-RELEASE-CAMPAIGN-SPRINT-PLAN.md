...
## Sprint-start record (2026-06-30)

- **Build number:** multi-element — IOSP **v1.0.0** (grabbed by `release-manual` at §4
  release time); app notes P2AN003/P2AN004 **v0.1.0**; the `mine-and-delineate` skill is
  unversioned.
- **Working tree (audit):** clean — all committed, no untracked source in the blast
  radius.
- **`tracking-readiness` entry check:** ran; board cleared — 12 shipped prior-sprint tasks
  (KB v1.13.0 + P2AN000→001 renumber) closed out + archived
  (`tasks/archives/archive_20260630_211822.md`). Cluster D folded (#47→§2/§4); #46/#54 kept
  as IOSP backlog; #110/#129 left (other elements); #140 CORDIC left (parallel Math track).
- **`baseline-health` entry baseline (GREEN):** YAML head — `verify-yaml-format.py`
  1124/1124 parse clean; `validate-crossref-keys.py` all cross-references resolved. IOSP
  document audit-state — committed-not-released, daemon-verified 382pp, with known pending
  corrections (RA-10 reversal / F-135) to fold in §2. *Exit comparison (sprint-closeout):
  YAML validators still green + IOSP released clean.*
...
## Section ↔ task cross-reference (generated by `plan-to-tasks`, 2026-06-30)

| Plan § | Deliverable | Task | seq |
|--------|-------------|------|-----|
| §0 | `mine-and-delineate` in-repo skill (foundational) | «#156» | 1 |
| §1a | USB study → IOSP augmentation (no app-note fork) | «#152» | 2 |
| §1b | P2AN003 DAC boundary determination | «#153» | 3 |
| §1c | P2AN004 Freq/Period/Pulse boundary determination | «#154» | 4 |
| §2 | single-writer merge → IOSP enrichment + release-depth re-audit | «#47» | 5 |
| §3a | author P2AN003 → verified PDF (v0.1.0) | «#157» | 6 |
| §3b | author P2AN004 → verified PDF (v0.1.0) | «#158» | 7 |
| §4 | IOSP release (release-manual, v1.0.0) | «#155» | 8 |

**Tag:** all eight carry `iosp-campaign`. **Order:** §0 → §1a/b/c (parallel fan-out) → §2
(barrier/merge) → §3a/b (doc-gen tail) → §4. **Out of this set:** «#46»/«#54» (IOSP
RECOMMEND_ADD / expert-Q backlog, dispositioned during §2 audit) and «#140» (CORDIC,
separate Math track) — tracked, not part of the campaign sequence.
