...
below. «#308» is a PROPOSED addition and is not approved yet._
...
| §8 / new | **Ingest `p2-hardware-manual` from the staged DOCX** — ⚠️ proposed, needs Stephen's yes | «#308» | 3 | 5h |
...
- **`p2-hardware-manual`** — §8's cross-check partner, and itself at **~65%** with no image
  ...
  corroboration. **«#308» proposes it; it widens scope, so it is Stephen's call (stop 1) and is
  not approved.** If declined, «#297» cross-checks against the 65% PDF-era extraction and
  anything it cannot corroborate twice stays out of the KB — a complete outcome, just a thinner
  one, and the closeout says so rather than letting thin read as full.
