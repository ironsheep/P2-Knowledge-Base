...
| `PUNCH_LIST_DOC` | **RESOLVED 2026-08-29** — the manual's own `engineering/document-production/workspace/<slug>/PUNCH-LIST.md`, **plus** the cross-cutting `engineering/document-production/PUNCH-LIST.md` for items spanning manuals (that file states the split itself). Read BOTH: central `sprint-plan` §1 presents the open items at the scope call and `sprint-retrospective` §5 triages them, so an unresolved sentinel silently empties two reads. | `engineering/operations/P2KB-CORRECTION-FINDINGS.md` | the source's `README.md` (ingestion dashboard) row + `sources/<src>/<src>-complete-extraction-audit.md` |
