...
4. **Code line-length gate.** Code boxes do NOT wrap, so an over-long code line is an authorship defect, not a template concern. Audit the **opus-master source** (so the reported `file:line` points at what the author edits — pass the single master file, or the chapter tree for a multi-file manual) against the manual's calibrated column budget K:
   ...
   - **Violations (exit 1)** — STOP. Relay the located `file:line: cols` list; the lines must be shortened in **opus-master** source before staging, by one of the sanctioned fixes: for a **comment** overflow, either (a) move the comment to full line(s) **above** the instruction at its indent, or (b) **split** it — keep the first part inline and put the overflow on the next line as a comment whose `'` is aligned to the same column as the inline `'` above it; for a **code** overflow, break at a logical boundary with the legal Spin2 `...` line-continuation (or aggregate into a named CON). Never a typeset wrap. Re-assemble and re-run. Do not escape/stage with violations outstanding.
     - 🔴 **EXCEPT inside a `` debug(`…) `` backtick directive, where BOTH of those fixes ship a different program.** A directive is a literal string assembled at compile time, not Spin2 source, so `...` is embedded **verbatim and everything after it is dropped**, and a `CON` symbol is embedded **unresolved**. Both compile clean; only a byte-compare of the `.bin` against the one-line form catches it. **A `debug()` line cannot be continued at all — it can only be shortened** (drop optional directive keywords the chapter teaches elsewhere · move a trailing comment above · shorten the instance name). Mechanism, evidence and the full sanctioned list: **project-overlay.md → "Augments Step 4"** (F-290).
...
6b. **Inline-code backtick-balance gate.** A single-backtick span whose **body contains a backtick** does not fail — it **inverts**. Pandoc closes the span at the first backtick it meets, so the next one *opens* a new span that runs on through ordinary prose, printing whole lines in monospace, fusing words, and flipping every span after it in the paragraph. **No other gate sees it:** the escaper hands the span through, the `.tex` is legal, the compile log is clean, and Step 4 measures fenced blocks, not inline spans. The only signal is an overfull box, which is exactly the kind of thing that gets explained away (F-294 — a whole "Try it" paragraph shipped wrecked in a released manual). Audit the **opus-master source** (same files as Step 4):
   ```bash
   python3 engineering/tools/validation/audit-backtick-balance.py \
       engineering/document-production/manuals/<slug>/opus-master/part-*/*.md
   ```
   - **Clean (exit 0)** — proceed.
   - **Violations (exit 1)** — STOP. Fix in **opus-master** by rewriting the span in the **double-backtick** form this set already uses for backtick-bearing content: ``` ``debug(`Waves TRIGGER 0 -500 500 256)`` ```. Re-assemble and re-run.
   - It is **paragraph-wise on purpose** — a code span may legally wrap across a newline inside one paragraph, and a line-wise check calls those broken when they are not.
   - ⚠️ **This gate was BUILT and then never wired into any skill** (found 2026-08-25, «#302»): the instrument F-294 declared missing existed on disk, clean, and nothing ran it. A gate nobody invokes is a gate that does not exist — which is the same shape as F-301's tracker that nothing read. It runs here, before a render is spent.
