...
- **Code that actually runs — yes, and this is where we're strongest.** Every code
  example is compiled clean on `pnut_ts` before it ships — itself a hundreds-of-tests-
  certified, 100% binary-compatible port of Chip's own PNut compiler, built with the
  chip's designer, so a clean compile there is a clean compile against the real thing. And
...
the knowledge base — add what's missing, or add the new wording, perspective, or
...
Tell us where it's wrong. That's what makes every edition that follows better — please let me know
when you do.
